...
public class LoginView {
    ...
    @FXML
    void clickBtnSubmit(ActionEvent event) throws Exception {
        UserPost user = new UserPost(loginInput.getText(),passwordInput.getText());
        if(SendPost(user)){
            switchToTable(event);
        }else {
            switchToAddUser(event);
        }
    }

    public boolean SendPost(UserPost user)  throws IOException {
        URL url = new URL("http://localhost:3307/users/check");
        ...
        boolean finalResponse = response.toString().contains("true");
    ...
    public void switchToAddUser(ActionEvent event) throws IOException{
        root = FXMLLoader.load(getClass().getResource("add-user.fxml"));
        stage = (Stage)((Node)event.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
